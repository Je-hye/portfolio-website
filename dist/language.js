(() => {
  ...
  const translations = new Map(Object.entries({
    ...
    "Working under my advisor's direction, I analyzed 2D evaluation experiment logs across all four codecs, reviewed CRF items and diffusion-model evaluation metrics, and selected and visualized worst-case results for the CODiff and DiQP restoration models across different QP conditions — running qualitative evaluation to see where restoration quality actually broke down. I also wrote a pipeline to auto-aggregate benchmark results, catching an anomalous log along the way.": "지도교수님 지시에 따라 4개 코덱 전반의 2D 평가 실험 로그를 분석하고, CRF 항목과 확산 모델 평가 지표를 정리했으며, CODiff·DiQP 복원 모델의 worst-case를 다양한 QP 조건에서 선별·시각화해 정성 평가로 복원 품질이 실제로 어디서 무너지는지 확인했습니다. 또한 벤치마크 결과를 자동 집계하는 파이프라인을 작성하는 과정에서 이상 로그를 발견했습니다.",
    ...
    "Set up a local LLM (Ollama) for anything sensitive, classified assets into exposure-risk tiers, and built a check-secrets.sh pre-tool-use hook plus a pre-commit hook and a standard .aiignore template — now in my global git init template and rolled out to 7 existing repos. Structural prevention, not cleanup after the fact.": "민감한 작업은 로컬 LLM(Ollama)으로 대응하고, 자산을 노출 위험 등급으로 분류했으며, check-secrets.sh 사전 훅과 pre-commit 훅, 표준 .aiignore 템플릿을 만들었습니다 — 현재 전역 git init 템플릿에 등록돼 있고 기존 레포 7개에도 적용했습니다. 사고 후 수습이 아니라 구조적으로 재발을 막는 방식입니다.",
    ...
    "UPGRADE DECISION": "고도화 판단",
    "This started as one script to draft a PR. I kept extending it on my own initiative — checkpointed state, then a QA pass, then a scheduler — until it became the daily system it is now.": "처음엔 PR 초안을 작성하는 스크립트 하나였습니다. 여기서 멈추지 않고 체크포인트 상태 저장, QA 단계, 스케줄러까지 스스로 범위를 넓혀가며 확장해, 지금의 매일 쓰는 시스템으로 만들었습니다.",
    "IN PROGRESS": "진행 중",
    ...
    "Currently splitting each role into its own tmux pane, so PM, Engineer, Reviewer, and QA run as visible parallel panes instead of one sequential thread.": "현재 각 역할을 개별 tmux 창으로 분리하는 작업을 진행 중입니다 — PM, Engineer, Reviewer, QA가 순차적으로 하나씩이 아니라 눈에 보이는 병렬 창으로 동시에 돌아가도록요.",
