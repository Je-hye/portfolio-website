(() => {
  ...
  const translations = new Map(Object.entries({
    ...
    "This started as one script to draft a PR. I kept extending it on my own initiative — checkpointed state, then a QA pass, then a scheduler, then a briefing agent — until it became the daily system it is now.": "처음엔 PR 초안을 작성하는 스크립트 하나였습니다. 여기서 멈추지 않고 체크포인트 상태 저장, QA 단계, 스케줄러, 브리핑 에이전트까지 스스로 범위를 넓혀가며 확장해, 지금의 매일 쓰는 시스템으로 만들었습니다.",
    "IN PROGRESS": "진행 중",
    ...
    "The briefing agent sorts incoming Gmail into five priority tiers with Claude Haiku before writing the daily digest, so it surfaces what matters instead of dumping every unread email.": "브리핑 에이전트는 매일의 요약을 작성하기 전에 Claude Haiku로 수신 Gmail을 5단계 우선순위로 분류해, 안 읽은 메일을 그대로 쏟아내는 대신 중요한 것만 먼저 보여줍니다.",
    "BLOG AGENT": "블로그 에이전트",
    "The blog agent doesn't write alone — it runs on a custom role-parallel writing skill from a separate skill repo, then drops the finished draft straight into the review loop above.": "블로그 에이전트는 혼자 글을 쓰지 않습니다 — 별도 스킬 레포에 있는 커스텀 역할 분담·병렬 작성 스킬로 초안을 만든 뒤, 완성된 초안을 위의 리뷰 루프로 바로 넘깁니다.",
    "Blog-writing skill ↗": "블로그 작성 스킬 ↗",
